    ...
    <div class="container" style="margin-top: 20px">
      ...
      <div v-if="items.length !== 0" class="row">
        <div class="col-12">
          ...
          <!--          body-->
          <div v-if="items" class="row" style="padding: 10px;font-size: 12px;text-align: left;">
            <div class="row" v-for="(item,index) in items" :key="'car'+index"
              ...
              <div class="col-2" style="text-align: center;margin-top: 10px">
                <button :disabled="item.num ===1?'disabled':false"
                        @click="$store.commit('numless',index),$forceUpdate()"
                ...
                <button @click="$store.commit('numadd',index),$forceUpdate()"
                        style="border: 1px solid #999999;margin:0 2px" class="d-none d-md-inline-block">+
                </button>
              </div>
              <div class="col-1" style="padding: 0;margin-top: 10px">¥{{ item.sum }}</div>
              <div @click="$store.commit('removeitem',index),$forceUpdate" class="col-1"
                   style="padding: 0;margin-top: 10px;color:#666;cursor: pointer">移除
              </div>
            </div>
          </div>
        </div>
